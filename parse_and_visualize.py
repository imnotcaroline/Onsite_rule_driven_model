...
XODR_FILE = "data/0_131_straight_straight_141.xodr"
...
def get_all_reference_points_of_one_road(geometries, step=0.01):
    ...
    for geometry_id, geometry in enumerate(geometries):
        ...
        # "position": (x, y)点的位置
        # "tangent": tangent_点的朝向
        # "s_geometry": s_点相对于其所属几何元素的局部距离
        # "s_road": point["s_geometry"]+s_start_road点相对于整条道路起点的全局距离
        # "index_geometry": geometry_id所属几何元素的索引
...
def get_width(widths, s):
    ...
    # 在OpenDRIVE中，宽度信息通常是分段定义的，每个< width > 元素都有一个起始的`sOffset`。
    # 排序是为了确保这些宽度定义按它们在道路上的出现顺序排列。
    widths.sort(key=lambda x: x.sOffset)
    current_width = None

    # milestones列表搜集了所有宽度定义（width）的sOffset值
...
def plot_planes_of_roads(total_areas, save_folder):
    ...
    # Plot boundaries 可视化车道区域，绘制车道边界
    ...
    # Plot center lane and reference line.绘制中心线和参考线
    ...
    # save_png_file = os.path.join(save_folder, "lanes.png")
    # print(save_png_file)
...
def process_one_file(file, step=0.1):
    ...
    d, ne = os.path.split(file)# d=onsite/outputB/test/0_131_straight_straight_141,ne=0_131_straight_straight_141.xodr
    n, e = os.path.splitext(ne)# n=0_131_straight_straight_141,e=.xodr
    save_folder = os.path.join(d, n)

    road_network = load_xodr_and_parse(file)
    total_areas = get_all_lanes(road_network, step=step)

    road_fig ,road_ax= plot_planes_of_roads(total_areas, save_folder)

    return road_fig,road_ax
...
if __name__ == "__main__":
    road_fig = None
    main("onsite/outputB/test/0_131_straight_straight_141/0_131_straight_straight_141.xodr")
